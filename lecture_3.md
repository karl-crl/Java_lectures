...
Исключением называют событие (или по-другому "исключительную ситуацию"), которое прерывает нормальное исполнение работы программы. Необходимо уметь понимать, к каким событиям программа "готова", а какие – являются исключительными, и не путать их друг с другом. Более подробно этому аспекту будет уделено внимание, когда пойдёт речь об [управлении на исключениях](#как-не-стоит-делать-при-работе-с-исключениями).
...
Все исключения в Java имеют тип `Throwable`. Они, в свою очередь делятся на исключения (`Exception`) и ошибки (`Error`).
